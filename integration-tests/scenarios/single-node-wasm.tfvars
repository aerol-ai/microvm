# Single-node WASM integration scenario: one mixed node with a public domain +
# TLS and the WASM runtime enabled via the config overlay (see
# single-node-wasm.caps.yml). Topology is identical to single-node.tfvars; the
# only difference between the two scenarios is the `wasm` capability, which
# run.sh turns into wasm.enabled = true + staged standard modules.
#
...
# config/secrets.yml.
cluster_name = "aerolvm-itest-single-node-wasm"
