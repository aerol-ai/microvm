...
For large fleet inventories, use the page-at-a-time API so the client does not
retain all 100,000+ sandbox records in memory:
...
<Tabs syncKey="lang">
  ...
  <TabItem label="Java">
    ...
    String token = "";
    do {
        var page = client.listPage(java.util.Collections.emptyMap(), false, token);
