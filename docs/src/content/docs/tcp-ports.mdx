...
### Why one method handles all three protocols

`(sandbox_id, port)` is the unique key on the exposure table, so there is at most one row per pair regardless of protocol - the daemon already knows the protocol from the row, the HTTP verb is the same (`DELETE /v1/sandboxes/{id}/ports/{port}`), and code that's defensive about not knowing how a port was published just works.

This mirrors `close(fd)` in POSIX: the system call is the same whether the descriptor is a TCP socket, a UNIX socket, or a pipe - the kernel knows which one it gave you.
