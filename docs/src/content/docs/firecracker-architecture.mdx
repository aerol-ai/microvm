...
- **RNG.** AerolVM does not attach a virtio-rng device. Cold-boot guests get
  ...
  signal below is wired to reseed the kernel RNG before any in-guest service
  runs, though the payload field that drives it is still a TODO.
...
- **Snapshot clones resume with stale entropy and clock.** Two clones of the
  ...
  `post_resume` signal so the in-guest agent can set the clock from the host.
  RNG reseed is wired on the agent side but not yet driven from the post-resume
  payload (`pkg/firecracker/snapshot.go:277`); cold boots skip this entirely
...
across a host - and what AerolVM still doesn't do today (UFFD, huge pages, RNG
reseed driven from `post_resume`).
