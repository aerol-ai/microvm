...
[Limitations](#limitations-and-gaps) for the entropy window before that ack
...
- **There is an entropy window before `post_resume` lands.** Reseed itself
  ships (`cmd/toolboxd/vsock.go:247` calls `ReseedRandom` on every
  `post_resume`), but the window between `Action(Resume)` and the agent's ack
  is hundreds of milliseconds on a slow host. Anything that runs in that window
  (init scripts, systemd units, the agent's own startup) draws from the
  template's captured entropy pool. Any in-guest CSPRNG that pins state at
  process start (TLS handshake nonces, session keys, IDs derived without
  re-reading `/dev/urandom`) before the ack **will** repeat values across
  clones of the same template. Templates AerolVM ships defer key generation
  until after the agent signals quiesce-complete.
