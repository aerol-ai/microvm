...
Register modules on the host before create, or let create resolve inline. WASM
catalogue rows are per-node and are not cluster-forwarded. Firecracker template
administration is different: it is an operator-only cluster control plane with
worker-routed create/item operations and a leader-coalesced list.
