...
:::caution[Operator-only control plane]
Template create, list, inspect, delete, and rebuild endpoints administer global
worker artifacts. They require operator credentials; managed tenant credentials
cannot use them. In cluster mode, an ingress routes each operation to the owning
Firecracker worker and coalesces list aggregation on the Raft leader.
:::
...
- **Cluster distribution.** Templates with a registry destination publish once to AOCR; peer workers pull artifacts lazily when placement needs them (see [Cluster Setup](/cluster-setup/)).
...
Pass an explicit `id` so retried CI steps are idempotent - the daemon rejects a duplicate id with `409 Conflict` rather than creating a second row. IDs must start with an alphanumeric character, use only alphanumerics, `.`, `_`, or `-`, and be at most 128 characters. The build runs asynchronously; the call returns immediately with `status: "pending"`.
