...
- `source` and `options` are **not secrets**. Read APIs return them, and in cluster mode they are replicated in the clear into the Raft placement spec and the recovery store so another node can re-run the mount. Only `credentials` is sealed and never replicated unencrypted, so every credential belongs there. A create request is rejected when a name that means credential appears in `options` (a key, an `extra_args` flag such as `--s3-secret-access-key`, or an NFS `opts` entry) or as an rclone connection-string parameter in `source` (for example `:s3,secret_access_key=...:bucket`); the error names the field to use instead.
