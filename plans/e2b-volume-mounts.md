...
- [x] **T6 (P1)** — daytona — full /volumes CRUD + reference-aware delete (409 while attached) ✅ DONE
  - Surfaced by: §3.8, §3.10, §5.5 — replace `volumesNotSupported`
  - Landed: **service-layer CRUD** (`internal/service/platform_volumes_crud.go`) — `CreatePlatformVolume` (idempotent get-or-create, quota), `GetPlatformVolume`/`ByName`, `ListPlatformVolumes`, `DeletePlatformVolume` (all gate on enabled→412); **reference-aware delete** via `platformVolumeAttacherCount`, which recomputes the volume's deterministic source (`volumes.MountSource`, shared with `BuildMountSpec` so they can't drift) and matches it against each live sandbox's unsealed mounts → `ErrPlatformVolumeInUse` (409) while attached. Daytona facade (`pkg/api/daytona/volumes.go` handlers + `routes.go` wiring) replaces all five 405 stubs; `resolveDaytonaVolumes` maps attach-at-create `volumes[{volumeId,mountPath}]` → tenant-scoped name → `serviceReq.PlatformVolumes`; wasm rejects volumes; removed dead `volumesNotSupported`/`errVolumesUnsupported`. Sentinel→HTTP: 412/409/404/400 (local `writeVolumeError` + create-path + `apihttp`). Tests: service CRUD/idempotency/quota/disabled + delete-no-attachers (UC-30) + **delete-while-attached 409** (UC-29) + release-then-delete; daytona routes 412-when-disabled, create-with-volume 412, `resolveDaytonaVolumes` validation. Updated 3 stale 405 tests.
  - New backing model: `internal/store` `volumes` table from T4; `volumes.MountSource` extracted as the shared source-of-truth.
- [x] **T7 (P1)** — sdk/models — `platform_volumes` field across models + 5 SDKs ✅ DONE
  - Surfaced by: §5.6, §7 Q3 — dedicated request field, no creds on the wire
  - Landed: `models.PlatformVolumeMount` + `CreateSandboxRequest.PlatformVolumes` (from T3) is the wire field. **TypeScript**: `PlatformVolumeMount` type + `platformVolumes` option + `toApiPlatformVolumeMount`. **Python**: `PlatformVolumeMount` TypedDict + `platformVolumes` + `_to_api_platform_volume`. **Go**: zero code — `CreateSandboxOptions = models.CreateSandboxRequest` alias already serializes it. **Rust**: `PlatformVolumeMount` struct + `platform_volumes` Option field + re-export. **Java**: `PlatformVolumeMount.java` model + `platformVolumes` field/setter. Per-SDK serialization test asserting `platform_volumes` round-trips with `read_only`. **All five verified locally** (TS `npm test`, Python `unittest`, Go `go test`, Rust `cargo test`, Java `mvn test`) — green.
