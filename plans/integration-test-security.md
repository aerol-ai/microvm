...
| T12 | UC groups A-D (sealing, failover, reseal, env) | T11 | **UC-117 green on S2** | **CODE DONE** 2026-09-26 (PR #481). Live exit criterion PENDING the gate run. Three plan corrections — no `secret.seal` event exists, UC-113 cannot replay the push, UC-121 is disruptive — see the findings box in §7. |
...
| T18 | Flagship run S5 + S6, publish reports | all | matrix in `reports/index.md` | **NOT STARTED.** Gate (S1→S4) running 2026-09-26; the flagship is the only remaining item. The on-demand vCPU quota is now 128, so the ~110 the hetero topology needs (c5.metal is 96) fits — the quota that blocked it has been raised. |
