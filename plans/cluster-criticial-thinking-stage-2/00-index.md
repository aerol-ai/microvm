...
The first-stage plan is directionally right, but too generous to the original
PR state. It correctly identifies the two big architectural problems:

- every node becomes a Raft voter;
- public sandbox URL traffic has no real owner-aware load balancer.

This branch now has first-slice fixes for target-locked create forwarding,
voter auto-promotion caps, FSM deep-copying, owner-aware public ingress, and
stable raw TCP route metadata. That is meaningful progress, but it is not the
same as a Kubernetes-grade release. The remaining critique is about the gap
between "functionally routed" and "production-grade at 200 x 50".

The plan still misses or underweights several release blockers:

- `POST /v1/sandboxes` has a target-locked fix, but the longer-term
  reservation-before-create model is still the stronger Kubernetes-like
  design.
- The branch now has owner-aware HTTP/TLS and raw TCP ingress, but it is an
  in-process per-node reconciler with route convergence windows, not yet a
  dedicated, watched ingress control plane with SLOs.
...
| [`02-release-blockers-in-current-pr.md`](./02-release-blockers-in-current-pr.md) | Concrete blockers found by comparing the plan to PR #58 plus notes on the fixes now present in this branch. |
